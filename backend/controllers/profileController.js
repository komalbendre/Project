  ...
  try {
    ...
    if (isOwner || isAdmin) {
      profileData.user = {
        ...
        fname: profile.userId.fname,
        lname: profile.userId.lname,
...
// ✅ GET /api/profile/me/profile - Get current user's profile
...
// ✅ POST /api/profile/:userId  — Create or update profile
// Also used by PUT /api/profile/me/profile (userId injected by route)
export const updateProfile = async (req, res) => {
  try {
    // FIX: fall back to the authenticated user's _id if param is missing/invalid
    const userId =
      req.params.userId &&
    ...
    if (technicalSkills) {
      if (typeof technicalSkills === "string") {
        technicalSkillsArray = technicalSkills
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s.length > 0);
    ...
    // Validate total skills limit
    if (technicalSkillsArray.length + softSkillsArray.length > 30) {
      return res.status(400).json({
        success: false,
...
export const getAllProfiles = async (req, res) => {
  try {
    ...
    const profiles = await Profile.find(query)
      .populate("userId", "fname lname email role isApproved createdAt")
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limit);
